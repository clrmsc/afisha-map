...
- `src/build.js` обходит все страницы листинга, собирает выставки, затем добывает
  координаты площадок (`AddressGeoPoint`) со страниц площадок и **кэширует** их
  в `data/venues.json` — при повторных запусках площадки не перезапрашиваются.
