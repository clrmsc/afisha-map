...
**1. Node.js и git** (если ещё нет):
```bash
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -
sudo apt-get install -y nodejs git
```

**2. Клонировать и установить одной командой:**
```bash
git clone https://github.com/clrmsc/afisha-map.git
cd afisha-map
bash deploy/install.sh
```
Скрипт сам определит путь и пользователя, поставит два сервиса (веб-сервер +
ежедневный парсинг), соберёт данные и включит автозапуск. В конце покажет адрес карты.

Настройки скрипта через env (необязательно):
```bash
PORT=8080 SCRAPE_TIME=05:00 bash deploy/install.sh   # свой порт и время парсинга
```

### Или вручную

Готовые unit-файлы — в [`deploy/`](deploy) (поправьте путь `/home/pi/afisha-map`
и пользователя `pi`), затем:
```bash
...
sudo systemctl enable --now afisha-map.service      # веб-сервер (автозапуск)
sudo systemctl enable --now afisha-scrape.timer     # парсинг каждый день в 06:30
```
...
Полезные команды:
```bash
systemctl status afisha-map                 # статус сервера
systemctl list-timers afisha-scrape.timer   # когда следующий парсинг
sudo systemctl start afisha-scrape.service  # запустить парсинг прямо сейчас
journalctl -u afisha-scrape.service -n 50   # лог парсинга
```
